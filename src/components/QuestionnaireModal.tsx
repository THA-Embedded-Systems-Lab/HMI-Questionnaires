...
const QuestionnaireModal: React.FC<QuestionnaireModalProps> = ({
  ...
  // Get Omega value for a specific scale and language
  const getOmegaForLanguage = (scaleName: string, language: string): string => {
    if (!questionnaire) return "N/A";
    ...
    const scale = dataEntry.scales.find((s) => s.name === scaleName);
    if (scale?.omega) {
      return scale.omega.value.toString();
    }
    return "—";
  };

  // Get Omega type for a specific scale and language
  const getOmegaTypeForLanguage = (
    scaleName: string,
    language: string
  ): string | null => {
                    ...
                    <tbody>
                      {Array.from(
                        ...
                        <tr key={index}>
                          ...
                          <td
                            className="omega-cell"
                            title={
                              availableLanguages.length > 0 && selectedLanguage
                                ? getOmegaTypeForLanguage(
                                    scaleName,
                                    selectedLanguage
                                  ) || undefined
                                : undefined
                          ...
                          >
                            {availableLanguages.length > 0 && selectedLanguage
                              ? getOmegaForLanguage(scaleName, selectedLanguage)
                              : "N/A"}
                          </td>
                        </tr>
                      ))}
